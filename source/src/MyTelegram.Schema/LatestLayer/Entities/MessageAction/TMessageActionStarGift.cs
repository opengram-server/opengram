...
public sealed class TMessageActionStarGift : IMessageAction
{
    ...
    public void Serialize(IBufferWriter<byte> writer)
    {
        ...
        if (Flags.IsBitSet(4)) { writer.Write(ConvertStars ?? 0); }
        if (Flags.IsBitSet(5)) { writer.Write(UpgradeMsgId ?? 0); }
        if (Flags.IsBitSet(8)) { writer.Write(UpgradeStars ?? 0); }
        if (Flags.IsBitSet(11)) { writer.Write(FromId); }
        if (Flags.IsBitSet(12)) { writer.Write(Peer); }
        if (Flags.IsBitSet(12)) { writer.Write(SavedId ?? 0); }
    }
