    ...
    IAmReadModelFor<StarsAggregate, StarsId, StarsRefundedEvent>
{
    ...
    public Task ApplyAsync(IReadModelContext context, IDomainEvent<StarsAggregate, StarsId, StarsAccountCreatedEvent> domainEvent, CancellationToken cancellationToken)
    {
    ...
    public Task ApplyAsync(IReadModelContext context, IDomainEvent<StarsAggregate, StarsId, StarsAddedEvent> domainEvent, CancellationToken cancellationToken)
    {
        Console.WriteLine($"[StarsReadModel] Applying StarsAddedEvent. PeerId: {PeerId}, OldBalance: {Balance}, NewBalance: {domainEvent.AggregateEvent.NewBalance}, Amount: {domainEvent.AggregateEvent.Amount}");
        Id = domainEvent.AggregateIdentity.Value;
        Balance = domainEvent.AggregateEvent.NewBalance;
        History.Add(new StarsTransaction
        {
        ...
        Console.WriteLine($"[StarsReadModel] StarsAddedEvent applied. New balance: {Balance}, Transaction added to history");
        return Task.CompletedTask;
    }

    public Task ApplyAsync(IReadModelContext context, IDomainEvent<StarsAggregate, StarsId, StarsSpentEvent> domainEvent, CancellationToken cancellationToken)
    {
    ...
    public Task ApplyAsync(IReadModelContext context, IDomainEvent<StarsAggregate, StarsId, StarsRefundedEvent> domainEvent, CancellationToken cancellationToken)
    {
        Id = domainEvent.AggregateIdentity.Value;
        Balance = domainEvent.AggregateEvent.NewBalance;
        History.Add(new StarsTransaction
        {
            ...
            Reason = $"Refund: {domainEvent.AggregateEvent.OriginalTransactionId}"
        });
        return Task.CompletedTask;
    }
}
